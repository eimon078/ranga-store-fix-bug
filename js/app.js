...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    <img class="product-image" src=${image}></img>
      ...
      <h4 class="topMargin">Price: <span class="text-light">$ ${product.price}</span></h4>
      <div id="rating-div">
        <p class="bottonMargin"><span><i class="fas fa-star checked"></i></span> ${product.rating.rate}</p>
        <p class="bottonMargin"><span><i class="fas fa-users checked"></i><span> ${product.rating.count} reviews</p>
      </div>
      <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="buy-now btn btn-success"><span><i class="fas fa-cart-plus"></i> </span>add to cart</button>
      <button id="details-btn" class="btn btn-danger"><span><i class="fas fa-info-circle"></i></span> Details</button>
      </div>
      `;
...
// Number of item select function 
let count = 0;
...
const updatePrice = (id, value) => {
  const oldProductTotalPrice = getInputValue(id);
  const currentProductPrice = parseFloat(value);
  const total = oldProductTotalPrice + currentProductPrice;
...
const updateTaxAndCharge = () => {
  const totalProductPrice = getInputValue("price");
  if (totalProductPrice > 500) {
    setInnerText("delivery-charge", 60);
    const totalTax = totalProductPrice * 0.4;
    setInnerText("total-tax", totalTax.toFixed(2));
  }
  else if (totalProductPrice > 400) {
    setInnerText("delivery-charge", 50);
    const totalTax = totalProductPrice * 0.3;
    setInnerText("total-tax", totalTax.toFixed(2));
  }
  else if (totalProductPrice > 200) {
    setInnerText("delivery-charge", 30);
    const totalTax = totalProductPrice * 0.2;
    setInnerText("total-tax", totalTax.toFixed(2));
  }
